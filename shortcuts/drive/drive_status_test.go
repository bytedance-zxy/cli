...
func TestDriveStatusCategorizesByHash(t *testing.T) {
	...
	// Local layout:
	//   local/a.txt   (content "aaa")  — also on remote with different content → modified
	//   local/b.txt   (content "bbb")  — only local                            → new_local
	//   local/sub/c.txt (content "ccc") — also on remote with same content     → unchanged
	// Remote-only:
	//   remote /d.txt                   — only remote                           → new_remote
	...
	// Subfolder list — distinguished by folder_token in URL.
	reg.Register(&httpmock.Stub{
		Method: "GET",
	...
	// Online docs and shortcuts must NOT leak into any bucket.
