...
func parseCommentReplyElements(raw string) ([]map[string]interface{}, error) {
	...
	for i, input := range inputs {
		...
		case "text":
			...
			if byteLen := len(input.Text); byteLen > maxCommentTextElementBytes {
				...
				return nil, output.ErrWithHint(
					...
					"split the content across multiple {\"type\":\"text\",\"text\":\"...\"} elements — the comment UI still renders them as one contiguous comment. The server returns an opaque [1069302] on overflow, so this check is pre-flight.",
				)
			}
